...
// EmailJS config — must be defined in `.env.local`.
// We support both the Vite-style `VITE_EMAILJS_*` keys already in the
...
export default function Contact() {
  ...
  const sendEmail = (e) => {
    ...
    // Guard: if EmailJS env vars are missing, surface a clear message.
    if (!SERVICE_ID || !TEMPLATE_ID || !PUBLIC_KEY) {
      setSnackbar({
        open: true,
        message: 'EmailJS конфигурацијата недостасува. Проверете .env.local.',
        severity: 'error',
      })
      return
    }
    ...
    emailjs
      ...
      .then(() => {
        setLoading(false)
        setSnackbar({ open: true, message: contact.snackbar.success, severity: 'success' })
      ...
      .catch((err) => {
        ...
        setSnackbar({
          open: true,
          message: `${contact.snackbar.error} ${err?.text || ''}`.trim(),
          severity: 'error',
        })
      })
  }
    ...
    <Box
      ...
      sx={{
        ...
        '&::before': {
          ...
          borderRadius: '50%',
          background:
            'radial-gradient(circle, rgba(134,46,156,0.1) 0%, transparent 65%)',
          pointerEvents: 'none',
        },
      }}
    >
        ...
        <Grid container spacing={{ xs: 4, md: 6 }} sx={{ alignItems: 'flex-start' }}>
          ...
          <Grid size={{ xs: 12, md: 4 }}>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {contact.info.map(({ icon, href, text }, i) => (
                <Box
                  key={text}
                ...
                >
                  <Box sx={{ color: '#862e9c', flexShrink: 0 }}>{renderIcon(icon)}</Box>
                  {text}
                </Box>
              ))}
            </Box>
            ...
            >
              <Grid container spacing={2}>
                <Grid size={{ xs: 12, sm: 6 }}>
                  <TextField label={contact.fields.name} name="from_name" required fullWidth variant="outlined" sx={FIELD_SX} />
                </Grid>
                <Grid size={{ xs: 12, sm: 6 }}>
                  <TextField label={contact.fields.email} name="reply_to" required fullWidth variant="outlined" type="email" sx={FIELD_SX} />
                </Grid>
              </Grid>

              <TextField label={contact.fields.subject} name="subject" required fullWidth variant="outlined" sx={FIELD_SX} />
              ...
              >
                {loading ? <CircularProgress size={22} color="inherit" /> : contact.submit}
              </Button>
            </Box>
          </Grid>
        </Grid>
