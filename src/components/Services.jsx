    ...
    <Box
      ...
      sx={{
        ...
        '&::before': {
          ...
          borderRadius: '50%',
          background:
            'radial-gradient(circle, rgba(134,46,156,0.07) 0%, transparent 70%)',
          pointerEvents: 'none',
        },
      }}
    >
      <Container maxWidth="lg">
        <Box sx={{ mb: { xs: 5, md: 7 }, textAlign: 'center' }}>
          ...
          >
            ...
            >
              {services.heading.split(' ').slice(1).join(' ')}
            </Box>
          </Typography>
        </Box>
        ...
        >
          {services.items.map((item, i) => (
            <ServiceCard
              key={item.title}
              service={{ title: item.title, desc: item.desc, icon: ICONS[i] }}
              index={i}
            />
          ))}
        </Box>
